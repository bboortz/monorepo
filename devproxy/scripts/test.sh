...
if [ -n "${CODECOV_TOKEN}" ]; then
  export LLVM_PROFILE_FILE="target/${OUTPUT_PROFILE}/prof/devproxy-%p-%m.profraw"
...
if [ -n "${CODECOV_TOKEN}" ]; then
  ...
  zip target/${OUTPUT_PROFILE}/prof/devproxy.zip target/${OUTPUT_PROFILE}/prof/*.profraw
  du -sh target/${OUTPUT_PROFILE}/prof/devproxy.zip
  ...
  # ./temp/grcov target/prof/devproxy.zip -s ./ -t html --llvm --branch --ignore-not-existing --ignore "/*" -o target/coverage -b target/debug
