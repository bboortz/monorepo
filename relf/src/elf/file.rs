...
#[deku()]
pub struct ElfFile {
    pub file_header: header::FileHeader,
}
...
impl Default for ElfFile {
    fn default() -> Self {
        Self {
            file_header: header::FileHeader::new(),
        }
    }
}
...
mod tests {
    ...
    fn test_from_bytes() {
        ...
        let (rest, val) = ElfFile::from_bytes((&data, 0)).unwrap();
        println!("rest: {:?}", rest);
        println!("value: {:?}", val);
    ...
    fn test_from_bytes_empty() {
        ...
        let err = ElfFile::from_bytes((&data, 0));
    ...
    fn test_try_from() {
        ...
        let val = ElfFile::try_from(data.as_ref()).unwrap();
        println!("value: {:?}", val);

        let expected = ElfFile::new();
        assert_eq!(expected, val);
