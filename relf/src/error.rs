...
mod tests {
    ...
    fn raise_err_trait_std_deku_error() -> std::result::Result<usize, error::Error> {
        let error_affected = String::from("file test.file");
        let error_suggestion = String::from("testcase");
        let deku_error = deku::error::DekuError::Parse(String::from("test error"));
        let error_type = error::ErrorType::Deku(deku_error);
        let err = error::Error {
            error_type: error_type,
            affected: error_affected,
            suggestion: error_suggestion,
    ...
    #[test]
    fn test_raise_err_trait_std_deku_error() {
        match raise_err_trait_std_deku_error() {
            Ok(_) => assert!(false, "Need to return an Error!"),
            Err(e) => {
                println!("{}", e);
                println!("{:?}", e);
